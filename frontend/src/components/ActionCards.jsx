  ...
  return (
    <div className="p-4 mt-8">
      <div className="grid gap-8 grid-cols-2 lg:grid-cols-5">
        {cards.map(
          ({ title, subtitle, Icon, href, gradient, hoverColor }, i) => (
            <Card
              key={i}
              title={title}
              subtitle={subtitle}
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        <Icon
          className={`absolute z-0 -top-8 left-52 size-22 opacity-40 rotate-5 ${hoverColor} group-hover:text-white group-hover:rotate-15 transition-all duration-300`}
        />
        <Icon
          className={`mb-2 size-10 ${hoverColor} group-hover:text-white transition-colors duration-300`}
        />
